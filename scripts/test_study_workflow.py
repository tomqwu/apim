...
'''
    ...
    def test_kong_guided_evaluation_contract_is_exact_and_falsifiable(self) -> None:
        ...
        self.assertRegex(styles, r"(?s)\.presentation-stage\.is-kong-guided\s*\{[^}]*--guided-control-cluster-width:\s*24rem;")
        self.assertRegex(styles, r"(?s)@media screen and \(min-width: 761px\).*?\.presentation-stage\.is-kong-guided \.journey-proof-strip\s*\{[^}]*padding-right:\s*calc\(var\(--guided-control-cluster-width\) \+ 2\.5rem\);")
        self.assertRegex(styles, r"(?s)@media screen and \(min-width: 761px\) and \(max-width: 1200px\).*?\.presentation-stage\.is-kong-guided\s*\{[^}]*--guided-control-cluster-width:\s*14\.375rem;")
        self.assertRegex(styles, r"(?s)@media screen and \(min-width: 761px\) and \(max-width: 1200px\).*?\.presentation-stage\.is-kong-guided \.slide-controls \.assessment-launch\s*\{[^}]*min-width:\s*46px;[^}]*width:\s*46px;[^}]*padding:\s*0\.35rem;")
        self.assertRegex(styles, r"(?s)@media screen and \(min-width: 761px\) and \(max-width: 1200px\).*?\.presentation-stage\.is-kong-guided \.assessment-launch > span\s*\{[^}]*position:\s*absolute;[^}]*clip-path:\s*inset\(50%\);")
