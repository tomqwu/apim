...
def fetch(base_url: str, relative: str, token: str, timeout: float) -> tuple[int, bytes]:
...
def verify_target(base_url: str, target: Target, token: str, timeout: float) -> None:
    status, data = fetch(base_url, target.relative, token, timeout)
    require(status == 200, f"{target.label} returned HTTP {status}")
    require(len(data) == target.size, f"{target.label} size differs from the manifest")
    require(digest(data) == target.sha256, f"{target.label} SHA-256 differs from the manifest")
...
def verify_once(
    ...
    root_status, root = fetch(base_url, "", token, timeout)
    ...
    missing_status, missing = fetch(base_url, missing_path, token, timeout)
...
def main() -> int:
    ...
    require(args.retries > 0 and args.delay >= 0 and args.timeout > 0 and args.workers > 0, "retry, delay, timeout, and worker values are invalid")
