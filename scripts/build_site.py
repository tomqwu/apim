...
# Public addresses at which this portal is served; rendered Markdown links to these addresses are
# localized to the serving host so previews and forks keep working. Extend only for real deployments.
PUBLIC_PORTAL_BASE_URLS = ("https://tomqwu.github.io/apim/",)
...
def clean_inline(value: str) -> str:
    ...
    # Inline code spans become spaces; never leave that space in front of trailing punctuation.
    value = re.sub(r" ([,;:.?!)])", r"\1", value)
    return value.strip(" -")
...
def guided_evaluation_visuals(
    ...
    duty_columns = ("Fit ID", "Outcome sought", "Why it fits", "Permanent duty", "Canonical duty record", "Counterfactual that would change the answer")
    ...
    duties = [
        {
            ...
            "record": clean_inline(row_value(row, "Canonical duty record")),
            "counterfactual": clean_inline(row_value(row, "Counterfactual that would change the answer")),
        }
...
def validate_site_projection(stats: dict[str, int], visuals: dict[str, object]) -> None:
    ...
    checks = {
        ...
        "Canonical figure contract text": (
            isinstance(kong_platform, dict)
            and isinstance(mule_migration, dict)
            and len(kong_platform.get("figures", [])) == 6
            and len(mule_migration.get("figures", [])) == 3
            and all(
                figure.get("depictedScope") and figure.get("accessibleEquivalent")
                for figure in [*kong_platform.get("figures", []), *mule_migration.get("figures", [])]
            )
        ),
        "Kong guided evaluation and native deck contract": (
            ...
            and all(
                row.get("outcome") and row.get("fit") and row.get("permanentDuty") and row.get("record") and row.get("counterfactual")
...
def build(output: Path) -> None:
    ...
    manifest = {
        ...
        "site": {
            "title": "API Management Studies",
            "description": "A living research, architecture, comparison, and proof-of-concept library.",
            "publicBaseUrls": list(PUBLIC_PORTAL_BASE_URLS),
        },
