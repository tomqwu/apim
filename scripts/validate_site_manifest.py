...
def validate_kong_guided_evaluation(
    ...
    require(
        len(questions) == len(canonical_question_semantics),
        "guided assessment question semantics must preserve the exact canonical count",
    )
    for question, canonical_semantics in zip(questions, canonical_question_semantics):
        observed_semantics = (
            question["id"],
            question["phaseId"],
    ...
    require(isinstance(reference_rows, list) and reference_catalog.get("rowTotal") == 14, "guided evaluation referenceCatalog must contain exactly fourteen slide groups")
