...
The vendor-neutral ownership choices, realistic scenario contracts, best practices, anti-patterns and acceptance evidence behind those tests are defined in the [industry-practices guide](45-api-management-industry-practices.md). This roadmap narrows that common contract to Kong mechanisms; it does not redefine the practices or lower their proof burden.
...
Traceable by Harness is not a new Kong option or a scored platform capability. If the security team admits the `GSA-01` adjunct hypothesis from the [guided evaluation](48-kong-guided-evaluation.md#traceable-by-harness-security-adjunct-feasibility), its plugin/agent mechanism becomes a dependent variable inside KMR-P03 trust and negative testing, KMR-P07 performance/scaling, KMR-P08 evidence/privacy/failure reconciliation, and KMR-P10 migration/exit. The study must freeze an exact Kong/plugin/TPA/EDS BOM and support position; trace request, response, security decision and prohibited-field paths; execute sync/async and fail-open/closed behavior; classify body/protocol/streaming coverage; measure latency/resources and agent scaling; rehearse upgrade/rollback/removal; and compare the result with the security team's Mule baseline. [Traceable Kong integration](https://docs.traceable.ai/docs/kong), [Harness WAAP plugin](https://developer.konghq.com/plugins/harness-waap/)
...
| if GSA-01 is admitted: exact Traceable plugin/TPA/EDS BOM, topology/support, data flow, protocol/payload/streaming coverage, fail-mode, performance/scaling, privacy, upgrade/rollback and removal contract | security architecture, platform product, Harness/Kong support, privacy and SRE | K1/K2 | exclude the adjunct from the target option and from any security, traceability or comparative conclusion |
