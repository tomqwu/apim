...
The generated public manifest contains only this question and choice-set definition. A conforming interaction layer keeps responses in volatile memory or browser-local storage until the participant explicitly exports them; it does not send responses to a server or write them into the repository. Do not capture names, email addresses, customer or organization identifiers, private topology, security findings, credentials, commercial quotes or terms, private URLs, raw logs, payloads, or restricted evidence. Use public role labels and controlled restricted-reference IDs only.
...
|---|---|
| Runtime storage | Keep response state in volatile memory or browser-local storage for the current user. Do not transmit, synchronize, publish, or write responses into the generated manifest or repository. |
...
| Local export | Export only on an explicit user action. Include schema/deck revision, session timestamps, question/phase/slide/target IDs, selected choice, public role labels, neutral notes, evidence-request fields, optional self-reported `evidenceLevel` (`E0`–`E4`) and `evidenceReference`, dissent, due gate, and next forum. |
| Evidence linkage | Export controlled canonical `criterion_id`, exact `option_id`, public evidence reference, and restricted-reference ID only when supplied. A response-level `evidenceLevel` or `evidenceReference` is a user-provided, unverified claim for gap triage: it never writes or upgrades the canonical evidence ledger and cannot change slide or criterion evidence. A response never writes `score` or `gate_status`. |
| Summary | Report approve/amend/hold, authorized and unauthorized scope, assumptions, evidence gaps, actions, dissent, and the next forum. Do not report a readiness percentage, inferred product confidence, or a rank from response counts. |
| Privacy | Reject or remove named-person assignments, personal data, customer or organization identity, private topology, security evidence, credentials, quotes or contract terms, private URLs, raw logs, and payloads. Use public roles and controlled restricted-reference IDs. |
