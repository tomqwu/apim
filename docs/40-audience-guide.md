...
The cross-role **Kong guided evaluation** is a separate 25-frame decision path for rooms that need the supplied preference model, conditional options, bounded Kong decision, architecture, adoption, migration, and production proof in one sequence. It begins with sanitized stakeholder inputs and an audited supplied scorecard, then switches to the canonical [Kong platform strategy](47-kong-enterprise-platform-strategy.md), [Kong option roadmap](44-kong-multicloud-study-roadmap.md), [Mule migration strategy](35-mule-migration-strategy.md), [Apigee A0–A6 migration roadmap](50-apigee-migration-strategy.md), [Kong terminology crosswalk](../research/glossary.md#kong-terminology-crosswalk), and [PoC evidence boundary](../poc/README.md). Its final appendix preserves the supplied comparison claims for audit, exposes multicloud, scalability/robustness, security/traceability, reversibility, fully allocated TCO, and control-plane responsibility as governed follow-up dimensions, and keeps all new weights and ratings `TBD`. The native Pages deck projects the current [guided-evaluation contract](48-kong-guided-evaluation.md). The repository PowerPoint remains the prior 2026-08-22 offline snapshot and is intentionally outside the 2026-08-24 page-and-document scorecard revision. Use the [facilitator guide](49-kong-guided-evaluation-facilitator-guide.md) as the complete meeting script: choose a timed route, deliver every slide's speaker notes, handle bounded side talks, record decisions and dissent, and return the room to the authorization or outcome gate.
...
7. Preserve the revised supplied score and govern any future recalculation; do not change criteria, weights, or ratings to make a preferred conclusion more convincing.
