...
| GEB-03 | Platform exit: non-Kong target | `KPS-P9` clean-room exit in docs/47; GEO-APIGEE, GEO-MULE, or GEO-APIM may serve as the non-Kong rebuild platform | Independent exit obligation: API contracts, routes and policies, consumers and credentials, PKI and identity, plugins, evidence history, and runtime must remain portable and are proved by a representative non-source rebuild with timed route-back. | True exit proof that no Konnect switch can close |
...
| Fit ID | Outcome sought | Why it fits | Permanent duty | Canonical duty record | Counterfactual that would change the answer |
|---|---|---|---|---|---|
| KPS-FIT-01 | Customer custody of management state | Aligns configuration authority, administrative evidence, backup, and recovery with the enterprise boundary | Operate and secure PostgreSQL, control-plane instances, backups, migrations, and private admin access; fund release engineering, compatibility testing, break glass, and reconciliation; carry the CP/PostgreSQL availability, PKI, backup/restore, licensing, plugin, upgrade, audit-evidence, support-coordination, and 24×7 response duty | docs/47 "Why self-managed control—and what it costs": Configuration/database custody and Change autonomy rows plus the operating-accountability paragraph; fit row KPS-FIT-01 in "Why Kong is the better fit here" | Control-plane custody is only a preference and managed lifecycle reduction is more valuable |
| KPS-FIT-02 | Multicloud runtime without a database in every zone | Places request enforcement near workloads while avoiding a regional Kong database per DP group | Design traffic, capacity, certificate, cache, and recovery per cell; prove every support, telemetry, license, registry, and backup flow; carry the 24×7 labour, database, Kubernetes/VM, network, telemetry, migration, and support cost | docs/47 "Why self-managed control—and what it costs": Failure-domain control, Sovereignty and forensic access, and Operating economics rows; fit row KPS-FIT-02 in "Why Kong is the better fit here" | Most workloads and dependencies consolidate in one managed cloud |
