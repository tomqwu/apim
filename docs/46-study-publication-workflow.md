...
1. wait for the `validate` workflow on the exact revision that Pages will deploy; normally this is the merge SHA, but a validated corrective follow-up may make it a clean descendant that still contains the intake merge and leaves every accepted canonical and file artifact byte-identical;
...
Record machine-checkable publication evidence, not prose placeholders: the three manifest assertions must be exactly `sourceRevision=<deployed-revision-SHA>`, `manifestSha256=<deployed-manifest-SHA-256>`, and `sourceDirty=false`; route assertions must be the exact `#/...` routes already declared in `derivedPaths`. The deployed revision must equal the intake merge SHA or be its clean descendant, remain reachable from `origin/main`, preserve every accepted canonical and file-like derived path byte-for-byte, and have successful `validate` and `pages` runs of its own. The published gate still authenticates the original merged PR and accepted candidate, then feeds every asserted route to the exact deployed-revision verifier and rejects missing, extra, or unknown routes. A corrective change to an accepted study or file artifact requires a superseding intake.

If main validation, Pages, or exact live parity fails, keep the record at `MERGED`, set `publicationStatus=corrective-change-required`, preserve the failing run/live evidence, and stop. Rerun source-preserving deployment controls first. With explicit repository-owner direction, a separate governed remediation may advance `main` only when the resulting revision cleanly descends from the intake merge and preserves every accepted canonical and file-like derived artifact byte-for-byte; that descendant needs its own green `validate` and `pages` runs and exact live proof. Any change to a protected intake artifact requires a superseding intake and cannot close this record. Do not rewrite shared history, reset `main`, fabricate closure, or merge another study publication while this record is open. This fail-closed limitation is intentional until the repository has an authoritative cross-intake lock/ruleset service.
