...
| Scope | Stakeholder decision posture to proceed with Kong Gateway Enterprise; conditional self-managed hybrid control-plane/data-plane target; customer-operated PostgreSQL, PKI, APIOps and multicloud data planes; federated application-repository delivery under centrally composed policy; bounded Kubernetes authority; RE-1 journeys; 0–18 month platform plan; Konnect operating-model benchmark and true cross-platform exit |
...
The most important strategic question is not whether the enterprise *can* host the CP. It is whether custody produces enough risk reduction and operating leverage to justify permanent database/platform duty. Self-management transfers **operating accountability and failure exposure** for CP/PostgreSQL availability, PKI, backup/restore, licensing, plugins, upgrades, audit evidence, support coordination and 24×7 response to the enterprise. That is an architecture and service-management conclusion, not a legal-liability determination; exact contracts, policy and qualified counsel must address legal allocation separately. The proof programme therefore compares self-managed hybrid and Konnect using the same workload, failures, support game day, staffing assumptions, and full cost. Preference alone is not a mandatory sovereignty requirement.
...
### Federated application-repository delivery

Federated delivery means that each application programming interface (API) team owns its contract, bounded API-specific intent, support metadata, and behavior tests in the service repository, while the platform team owns the versioned controls and the only deployment identity permitted to write to a Kong Control Plane (CP). It does **not** mean that every repository runs decK against Kong, copies centrally governed plugins, or receives permission to weaken mandatory controls. The application repository is a source authority for its bounded objects; the central application programming interface operations (APIOps) pipeline is the single runtime writer.

The word *application* has two meanings that must not be conflated. An **application repository** is the producer-side source for an API. A **consumer application** is a portal/product identity that requests and receives runtime access. Producer delivery can be federated without moving consumer registration, credentials, product approval, or revocation into producer repositories.

| Governed object | Application-team repository owns | Central platform owns | Composition and one-writer rule | Required evidence |
|---|---|---|---|---|
| OpenAPI contract and version | Paths, methods, schemas, examples, compatibility intent, business owner and lifecycle state | Contract standards, lint rules, naming and compatibility policy | The application repository is source authority; the platform pipeline rejects an invalid or incompatible contract | Application commit, validation report and contract digest |
| Kong Service and Route intent | Application programming interface (API) identifier, approved host/path request, safe backend reference, timeouts within published bounds and API-specific tags | Host/path reservations, environment boundaries, route-safety rules and entity-writer registry | The application repository proposes intent; only the platform deployment identity creates or changes runtime entities | Composed Service/Route, collision report, target Control Plane (CP) and active digest |
| API behavior proof | Contract, routing, negative and domain-outcome cases | Minimum corpus, fault cases, test harness and evidence schema | Application cases cannot replace platform security or recovery tests | Versioned test corpus, result bundle and accountable reviewer |
| Mandatory gateway controls | Selects only documented, application-selectable profiles and values inside approved bounds | Authentication, transport authorization, logging, tracing, correlation, privacy controls, plugin catalog, versions and defaults | A versioned central policy bundle is composed at validation/deployment; reserved plugins and mandatory settings cannot be copied, removed or weakened by application intent | Policy-bundle version, prohibited-override report and effective configuration |
| Consumer, product and credential lifecycle | Declares intended audience and product linkage as metadata, where allowed | Consumer, Consumer Group, credential, product, quota, approval, recertification and revocation standards | Portal/product authority remains central; producer Git does not issue or revoke consumer credentials | Product/application record, approval, effective allow/deny and offboarding reconciliation |
| Trust and sensitive references | Uses approved logical references only | Identity profiles, certificates, public key infrastructure (PKI), vault policy, trust rotation and secret delivery | Values are resolved by the platform boundary; sensitive material never enters the application repository or generated review diff | Reference-resolution result, trust epoch and negative authorization test |
| Platform topology and access | Selects an approved environment/runtime class | Control Plane/Data Plane topology, Workspaces, role-based access control (RBAC), network, Terraform modules, decK version and deployment permissions | Application repositories cannot choose an unmanaged target or call the Admin API | Target identifier, deployment identity, infrastructure release and native receipt |
| Exceptions | Supplies business reason, bounded scope and removal owner | Defines exception-eligible rules, approver roles, compensating controls, maximum duration and audit retention | Non-exemptible controls remain mandatory; an approved exception is narrow, reviewed, time-bounded and rejected after expiry | Exception ID, reviewer role, expiry, compensating-control test and closure record |
| Release and active state | Supplies the immutable application commit | Signs the release envelope, runs reviewed plan/apply and reconciles active state | One platform release service writes each entity; break glass is a separate expiring lease followed by reconciliation | Full evidence chain described below |

The standard flow is fail-closed:

1. The application pull request validates the OpenAPI contract, required owner/support metadata, route intent, operation coverage and application-selectable plugin bounds.
2. The platform pipeline resolves exactly one writer for every Service, Route and plugin instance; rejects duplicate names, conflicting host/path claims, prohibited plugins, unsafe routes, missing owners and stale exceptions; and selects a versioned central policy bundle.
3. A deterministic compiler composes application intent with central policy. Mandatory settings take precedence by construction, not by review convention, and the effective configuration is visible before approval.
4. A platform-owned identity runs decK validation and a scoped diff or equivalent deployment plan. A reviewer approves the complete create/change/delete set for the named target CP; application workflows cannot apply it directly.
5. After deployment, the pipeline captures the active configuration, runs outside-in behavior and negative controls, and binds the result to the immutable evidence chain.
6. Continuous reconciliation compares desired and active digests. Drift, an unknown writer, an unapproved entity, or a digest mismatch opens an incident or bounded reconciliation workflow rather than silently overwriting state.

| Evidence-chain element | Binding rule | Failure meaning |
|---|---|---|
| Application commit | Immutable Git commit for contract, intent, metadata and tests | The producer input cannot be reconstructed |
| Central policy version | Immutable bundle identifier and Secure Hash Algorithm 256-bit (SHA-256) digest | The mandatory controls applied to the release are unknown |
| Generated Kong configuration | Deterministic digest plus entity-level provenance | The reviewed plan cannot be tied to the intended configuration |
| Reviewed decK diff or equivalent plan | Digest, reviewer role, target, decision and review time | Create/change/delete impact was not independently approved |
| Target Control Plane (CP) | Stable environment and CP identifier | Evidence may belong to the wrong lifecycle boundary |
| Active configuration digest | Post-deployment capture plus convergence result | Desired state is not proven active, or configuration drift exists |

The public-safe [federated delivery reference](../poc/federated-api-delivery/README.md) implements this boundary with synthetic OpenAPI input, application intent, a central policy bundle, a writer registry, exception rules, an offline deterministic compiler, negative controls, a reviewable plan and active-digest attestation. It is executable harness evidence only; it is not a decK run, a Kong deployment result, or proof that the target option passes.
...
| GP-5 federated change promotion | Application Git intent + versioned central policy → lint/semantic/resource checks → deterministic composition → decK validate/diff → canary cell → reconcile | Mandatory controls cannot be weakened; prohibited plugin, unsafe route, missing owner, conflicting writer, stale exception and drift fail closed; scoped deletion, signed release, stop/kill switch, no manual permanence | Application pull request shows the effective policy, entity impact and target without granting direct Control Plane write access | Application commit, policy version, generated-config digest, reviewed diff, target Control Plane, native receipt, active digest and outside-in probe |
...
| GP-7 third-party security adjunct | Admit a versioned adjunct such as Traceable only for an approved, bounded security/traceability use case | Certified exact BOM; plugin/agent data path; fail-open/closed rule; protocol/payload coverage; privacy; latency/resource budget; HA/scaling; upgrade/rollback/uninstall; joint support RACI | One template exposes mode, failure, capture, privacy, owner and safe-removal choices rather than hiding vendor defaults | Raw capture/enforcement, negative, fault, load, gap, DLP, lifecycle, support and route-back evidence |
...
| KP1 — build the reversible foundation | 2–5 months | Non-production self-managed platform with evidence-producing delivery | CP/PostgreSQL management cell, private Admin/RBAC/audit, PKI, vault, application-repository/central-policy composition, writer registry, Git/decK path, first DP cells, collectors, release manifest | Clean deterministic build, policy/version provenance, scoped sync/rollback, per-DP hash, native restore, negative access result | Mandatory override, prohibited plugin, unsafe route, missing owner, conflicting writer, unrecoverable DB, secret leakage, false-ready node, or unbounded global change |
...
| KP3 — launch paved roads and migration cell | 6–10 months | Safe self-service for bounded non-critical APIs and one representative hard pattern | GP-1–GP-6 templates, federated producer onboarding, central policy and exception lifecycle, catalog/ownership reconciliation, domain enablement, coexistence, consumer lifecycle, cost telemetry | Producer-to-runtime release envelope, persona journey, API owner, active-state evidence, route-back and support readiness | Teams bypass composition, direct CP writers appear, evidence burden is manual, portal/runtime access drifts, or ownership seam is unstaffed |
...
| KO-5 safe change | Change-failure, detection/stop/recovery time, propagation distribution, drift and policy-bypass attempts | Zero dual writer, mandatory-control override, prohibited plugin, unsafe route or destructive surprise; fleet converges within approved objective | Application commit, central-policy version, generated configuration, reviewed decK diff/result, target CP, active digest, canary trace and rollback/forward-fix | Every release; weekly trend | Platform product/release engineering |
...
| KO-7 security traceability and evidence safety | Release/config/request/security/business correlation; request impact; queue age; produced/queued/dropped/sent signals; privacy findings | Every required decision is attributable and correlated; optional telemetry remains inside approved request-impact envelope; loss/gap quantified; zero prohibited field | Release/config identifiers, gateway/adjunct/collector raw metrics, security decisions, trace/log/business join, DLP scan, incident query and gap declaration | Continuous; each failure game day | Observability + security operations |
...
7. Federate producer intent, not runtime authority: application repositories own contracts and bounded API intent; versioned central policy is composed by one platform writer, and every release must reconcile to an active digest.
8. Compare self-managed with Konnect continuously using the same outcomes. Separately maintain and rehearse a true non-Kong exit.
9. Scale only after two representative workload patterns produce independently reviewed E4 evidence, including an upgrade/rotation, incident/support interaction, rollback, and cost/toil record.
...
|---|---|---|---|---|---|
| KPS-P1 exact option and custody | Freeze image/plugin/PostgreSQL/decK/Kubernetes/support BOM; trace payload/config/credential/telemetry/backup/support/operator fields | Unresolved fields; prohibited/unknown flows; E2 gaps | Zero unresolved field that changes a mandatory mechanism; zero prohibited/unknown mandatory flow | Immutable option record, SBOM/digests, field ledger, contract references | Architecture/privacy/third-party-risk panel |
| KPS-P2 configuration truth and federation | From an application commit, compose a versioned central policy bundle; omit owner; collide writer/route; request a prohibited plugin, unsafe route, mandatory-control override or expired exception; run reviewed decK diff/scoped sync, canary, stop, restore and active-state reconciliation | Rejection accuracy, deletion/collision, affected traffic, stop/recovery, desired/active digest and per-DP hash/age | Every unsafe case rejected before apply; zero destructive surprise, dual writer, policy weakening or false complete; all serving DPs map to one accepted release | Application commit, policy version/digest, generated config and provenance, reviewed diff, target CP, native receipt, active digest, audit, runtime inventory and request trace | Independent release engineering + security/governance reviewer |
...
| KPS-P5 trust and security adjuncts | OAuth/JOSE negative corpus; attempt a DP connection using an unlisted certificate signed by the trusted cluster CA; rotate issuer, partner, backend and CP/DP certs and the CN allowlist; quarantine a named DP; deny IdP/vault; test expiry/revoke/clock; if GSA-01 is admitted, execute Traceable sync/async, allow/fail behavior, TPA/EDS loss and policy-negative cases | Unauthorized success, same-CA DP admission, allowlist propagation, trust age, rotation/quarantine time, adjunct decision/failure, leakage, attribution | Zero unauthorized/cross-tenant success, zero unlisted same-CA DP admission, and zero prohibited secret/field; degraded and adjunct failure behavior within approved rule | Token/cert/handshake and CP connection logs, allowlist/config digest, vault/cache, adjunct decision/path, config identity, audit and DLP bundle | Independent security/IAM/PKI tester |
| KPS-P6 capacity, counters and evidence | Run RE-1 ordinary/busy/burst mix; lose node/zone; add Redis and collector or admitted security-adjunct latency/partition/saturation; slow client/upstream | Journey SLO/correctness, saturation, quota divergence, retry amplification, trace correlation, signal loss, adjunct overhead and unit cost | Critical outcome holds; degradation explicit/bounded; no optional telemetry/security adjunct cascade; gaps reconciled | Load/fault raw data, counters, resources, gateway/adjunct/collector reconciliation and cost model | Performance/resilience + business-risk panel |
...
| KPS-P9 migration and true exit | Move gateway- and integration-dominant slices; for admitted GSA-01 remove/disable the Traceable plugin and agents; lose response; route back; clean rebuild without old CP on another platform | Semantic/business variance, missing entities/history, credential work, adjunct removal, rollback, residual dependencies | Zero unexplained critical variance; every layer classified; security behavior remains explicit; safe route-back and dependency-zero exit | Compatibility corpus, state/consumer/adjunct ledger, export/rebuild diff, route-back, removal and decommission record | Domain/data/integration/security architecture assurance |
...
- A documented third-party security plugin is neither a native platform outcome nor a certified target integration. It can add local agents, inline failure choices, data-processing paths, resource overhead, upgrade/support seams and exit work that require separate admission.

## Open evidence requests

| Evidence request | Owner role | Due gate | Decision impact if absent |
|---|---|---|---|
| Exact Gateway/PostgreSQL/OS-or-Kubernetes/image/chart/decK/plugin/license BOM and Kong support statement | Kong technical owner + platform engineering + sourcing | KP0/KP1 | Target remains unresolved; no E3 or production admission |
...
| If GSA-01 is admitted: exact Kong 3.14/Traceable plugin/TPA/EDS version, checksum/SBOM, topology, support, data-processing, protocol/payload/streaming, fail-mode, scaling, upgrade/rollback and uninstall terms | Security architecture, platform product, Harness/Kong support, privacy and sourcing | KP1/KP2 | Do not admit the adjunct, claim parity, or use it in a production/security recommendation |
...
| Federated producer ownership matrix, versioned central policy schema, prohibited-plugin/unsafe-route rules, exception contract, application repository template, and immutable release-evidence schema | API product + platform release engineering + security/governance | KP1 | Producer self-service remains blocked; application repositories cannot write to a CP |
