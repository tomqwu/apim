...
- Record exactly `sourceRevision=<deployed-revision-SHA>`, `manifestSha256=<digest>`, and `sourceDirty=false` as manifest assertions. The deployed revision must equal the intake merge SHA or be a clean, validated descendant that contains it, preserves every accepted canonical and file-like derived artifact byte-for-byte, and has its own successful `validate` and `pages` runs. Record every and only declared `#/...` derived route as route assertions so the live verifier checks them mechanically while the original merged PR and accepted candidate remain independently authenticated. If a descendant changes an accepted study or file artifact, create a superseding intake instead of closing the original one.
